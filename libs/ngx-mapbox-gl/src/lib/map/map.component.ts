 ...
 */
export type MapComponentInitializationOptions = MapOptions;

/**
 * InjectionToken used for injecting default options to the MapComponent before it initializes.
 */
export const MAP_COMPONENT_INITIALIZATION_OPTIONS =
  new InjectionToken<MapComponentInitializationOptions>(
    'MapComponentInitializationOptions',
  );
...
export class MapComponent implements OnChanges, OnDestroy {
  ...
  cooperativeGestures = input<MapOptions['cooperativeGestures']>(
    this.initialMapOptions?.cooperativeGestures,
  );
  ...
  scrollZoom = input<MapOptions['scrollZoom']>(
    this.initialMapOptions?.scrollZoom,
  );
  dragRotate = input<MapOptions['dragRotate']>(
    this.initialMapOptions?.dragRotate,
  );
  touchPitch = input<MapOptions['touchPitch']>(
    this.initialMapOptions?.touchPitch,
  );
  touchZoomRotate = input<MapOptions['touchZoomRotate']>(
    this.initialMapOptions?.touchZoomRotate,
  );
  doubleClickZoom = input<MapOptions['doubleClickZoom']>(
    this.initialMapOptions?.doubleClickZoom,
  );
  ...
  zoom = input<[number] | number | undefined>(this.initialMapOptions?.zoom);
  bearing = input<[number] | number | undefined>(
    this.initialMapOptions?.bearing,
  );
  pitch = input<[number] | number | undefined>(this.initialMapOptions?.pitch);
  ...
  fitBoundsOptions = input<MapOptions['fitBoundsOptions']>(
    this.initialMapOptions?.fitBoundsOptions,
  );
  renderWorldCopies = input<MapOptions['renderWorldCopies']>(
    this.initialMapOptions?.renderWorldCopies,
  );
  projection = input<MapOptions['projection']>(
    this.initialMapOptions?.projection,
  );
